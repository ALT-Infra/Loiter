...
The current interface sources are `canvas.template.html`, `canvas.css`, `canvas.js`; `canvas_data.py` enriches the original inventory. `atlas.template.html` and `check_browser.mjs` are retained legacy sources and are not used by the current build. Typeface license notices are embedded and included in `fonts/`.
